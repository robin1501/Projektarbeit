...
/**
 * Die HeadOfDepartment Klasse beschreibt die Möglichkeiten eines Studiengangsleiter in der Studentenverwaltung.
...
public class HeadOfDepartment extends User implements ILecturer, IProf, IHead {
	...
	public double getLectureAverage(String SelectedLecture) {
		...
		int count=0;
		for(int i=0;i<data.size();i++){
			if(data.get(i).get(2).toString().equals(SelectedLecture) && data.get(i).get(2).toString().equals(course)){
